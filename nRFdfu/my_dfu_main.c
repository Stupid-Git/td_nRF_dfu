...
bool serial(char *package, char *port, int baudrate, bool flowcontrol)
{
    ...
    serial_medium_t *medium;
  //medium = serial_medium_Null_instance();
  //medium = serial_medium_NW_instance();
    medium = serial_medium_UW_instance();

    // TODO: Look into using click.progressbar to show progress during DFU
    serial_backend = DfuTransportSerial(medium, port, baudrate, flowcontrol);
...
bool my_cli_serial()
{
    ...
    package = "kjlkjkljkljl";
    port = "\\\\.\\COM56";
    baudrate = 38400;
    flowcontrol = true;
    
    bret = serial(package, port, baudrate, flowcontrol);
    return(bret);
}
