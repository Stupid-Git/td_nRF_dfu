...
struct CL_Dfu; //FORWARD
typedef struct CL_Dfu Dfu_t; //FORWARD

typedef void (*xxx_event_handler_del)(Dfu_t *P, char*);
...
typedef int32_t (*CL_send_start_dfu_del)   (DfuTransport_t *pT, int program_mode, int softdevice_size, int bootloader_size, int app_size);
typedef int32_t (*CL_send_init_packet_del) (DfuTransport_t *pT, buf32_t *init_packet);
typedef int32_t (*CL_send_firmware_del)    (DfuTransport_t *pT, buf32_t *firmware);
...
typedef struct CL_DfuTransport
{
    ...
    */
    ...
    int32_t (*SET_DEVICE_DFU_MODE_ON) (DfuTransport_t *pT);
    int32_t (*SET_DEVICE_DFU_MODE_OFF)(DfuTransport_t *pT);
...
{
    DfuTransport_t  DfuTransport; //must appear first

    serial_medium_t *serial_medium;
